...
function executeRun(run) {
  return new Promise((resolve) => {
    ...
    // On Windows, resolve the full path to claude.exe so we can avoid shell: true
    // (shell: true causes cmd.exe to split multi-word -p arguments)
    let bin = CLAUDE_BIN;
    let useShell = false;
    if (IS_WIN) {
      try {
        bin = execSync('where claude', { encoding: 'utf-8' }).trim().split('\n')[0].trim();
      } catch {
        // Fall back to shell: true if 'where' fails
        useShell = true;
      }
    }
    ...
    function gracefulKillProc(p) {
      if (IS_WIN) {
        // Windows: no SIGTERM equivalent — force kill with tree
        forceKillProc(p);
      } else {
        p.kill('SIGTERM');
...
function killCurrent() {
  ...
  if (IS_WIN) {
    try { execSync(`taskkill /PID ${currentProcess.pid} /T /F`, { stdio: 'ignore' }); } catch {}
  } else {
