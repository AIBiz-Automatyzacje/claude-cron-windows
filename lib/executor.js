...
const { CLAUDE_BIN, WORKSPACE_DIR, MAX_LOG_SIZE, IDLE_TIMEOUT_MS, WATCHDOG_INTERVAL_MS } = require('./config');
const db = require('./db');
  ...
  return new Promise((resolve) => {
    ...
    const watchdogId = setInterval(() => {
      if (Date.now() - lastChunkAt > IDLE_TIMEOUT_MS) {
        diagLog += `${ts()} WATCHDOG: wall-clock idle detected (last chunk ${((Date.now() - lastChunkAt) / 1000).toFixed(1)}s ago)\n`;
        killProc('idle');
      }
